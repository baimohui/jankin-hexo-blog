...
**例 11：**
```
在 JavaScript 中，下面选项关于 this 描述正确的是（A、D）
...
const A = obj.method;
A();  // 这里 this 是全局对象（非严格模式）或 undefined（严格模式），而不是 obj
```
- 函数赋值给变量后，调用时丢失了原本的对象上下文，`this` 取决于调用方式（这里是独立函数调用）。
- 所以 B 的 **错误**。

---

**C. 在函数定义时，this 指向全局变量**  
- `this` 是在**函数调用时**才确定的，不是在定义时确定。
- 只有箭头函数在定义时捕获其所在上下文的 `this`，但普通函数是在调用时决定。
- 所以 C 的 **错误**。

---

**D. 在浏览器下的全局范围内，this 指向全局对象**  
- 浏览器全局范围（不在任何函数内）的 `this` 指向 `window`（即全局对象）。
- 这是对的（Node.js 中是 `global`，但题目明确“浏览器下”）。
- 所以 D 的 **正确**。
