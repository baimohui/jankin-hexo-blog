...
6. 【Vue】动态绑定样式
   ...
   <!-- 数组中嵌套对象 -->
   <div
     :class="['class1', 'class2', { class3: false || flagData }]"
   >content</div>
   <!-- 对象 -->
...
33. 【Vue】ref 值为动态时，可以通过 this.$refs[ref]来获取。注意子组件 data 值要在`this.$refs[ref][0].data`才能获取到。

34. 【Vue】子组件可以使用 watch 来监听 props 值变化。

35. 【Vue】articleList.vue：watch 用&&而非 if 报：Vue Error－报错信息：Syntax Error: Assigning to rvalue.

36. 【Vue】子组件通过 emit 向父组件传值：父组件监听的方法不要加 ()

37. 【Vue】在 js 中修改 checkbox 的 checked 会报 Vue:Avoid mutating a prop directly since the value will be overwritten。所以不能直接修改。但可以在 template 中，:checked=xxx，通过改变 xxx 的值来改变 checked 的值。

38. 【uniapp】Hbuilder 真机调试时提示“缺少平台 app-plus 插件”：npm install @dcloudio/uni-app-plus --save

39. 【JavaScript】indexOf() 妙用：结合 splice(indexOf, 1) 删除一个元素

40. 【CSS】iphone5 绝对定位偏移问题：https://www.jianshu.com/p/e7360e8de461

41. 【uniapp】uni.navigateTo 跳转到非 tabbar 页面，uni.switchTab 跳转到 tabbar 页面

42. 【uniapp】iconfont 的 unicode 引入方式有差异：`&#xe63f;` `\ue63f`

43. 【uniapp】image 的 mode 坑 https://uniapp.dcloud.io/component/image?id=image

44. 【CSS】border-radius: (左上，右上，右下，左下)

45. 【CSS】left 和 right 也要加，不然会没有宽度
...
47. 【CSS】文本下半部分显示背景色（貌似不可实现）：https://bbs.csdn.net/topics/391824450

48. 【CSS】margin-top 为负值可让元素上移

49. 【git】拉取某一分支的代码：git clone -b <分支名> 仓库地址
...
51. 【小程序】`project.config.json` 中配置 `"miniprogramRoot": "./dist/dev/mp-weixin"`，即 `app.json` 所在的目录

52. 【Linux】xshell 发布：登录跳板机，切换到 192.168.3.111 机器，切换到 root 权限（sudo -i），发布 H5 页面：`sh /data/code/linkkap/linkkap-diagnosis-h5/shell/build.sh`

53. 【JavaScript】当滑动到某一位置才显示元素：懒加载判断：`getBoundingClientRect()`
...
55. 【CSS】背景图片按钮发生闪烁：https://www.cnblogs.com/yinzhengjie/p/8605906.html

56. 【JavaScript】setInterval 的函数参数中的 this 指向不是全局变量

57. 【小程序】页面`onShareAppMessage() {}`定义分享

58. 【JavaScript】axios 使用 token 发送请求，避免了 csrf 攻击。而且双重 token 验证

59. 【Vue】checked 属性大坑：只要给输入元素添加 checked 属性，无论其值是 true 还是 false，都会是选中状态。disabled 同理

60. 【JavaScript】对 test1 的修改会影响到 test0：对象和数组都保存在堆中。
...
61. 【Vue】在 data 中调用另一个 data 需要添加 this 会出错（待查明原因），不添加则指向 export default 外创建的变量。一般要依赖另外一个 data 的情况，都是放到计算属性里。

62. 【JavaScript】数字转为字符 `Number.toString()`、`String(Number)`

63. 【Vue】禁止页面滚动 / 滚动事件穿透 (弹出蒙版时，弹出层下面还可以滚动：@touchmove.prevent) https://blog.csdn.net/weixin_43106777/article/details/90268333
    另外一种方法是给页面的高度定死，待实践

64. 【Vue】input 保持焦点 focus：https://www.cnblogs.com/wangdashi/p/9660747.html

65. 【Vue】获取 input 输入值：https://www.cnblogs.com/zbl3033/p/13394855.html

66. 【ApiCloud】添加新页面后要重新运行

67. 【JavaScript】async 与 await 配对问题（forEach)https://blog.csdn.net/april_4/article/details/85336366

68. 【Vue】@scroll 无法监听：https://segmentfault.com/q/1010000009119633

69. 【Vue】如果图片地址是本地路径，怎么对 img 的 src 动态赋值 https://www.jianshu.com/p/6ee27c825025

    `<img :src="../../assets/images/classic.jpg">`

    ` <img :src="require('../../assets/images/classic.jpg')" alt="picture">`

70. 【JavaScript】深拷贝在项目中的应用：打印输出，确定精确值

71. 【Vue】动态组件和 v-if 效果比对。uniapp 不能使用动态组件。

72. 【浏览器】Provisional headers are shown. 这个问题字面意思是“显示了临时报文头”，浏览器第一次发送这个请求，请求被阻塞，未收到响应。当要求浏览器再次发送这个请求时，上个同样的请求都还没有收到响应，浏览器就会报这个警告。简而言之，就是**请求并没有发出去**。谷歌不行，用其它浏览器打开就 ok。

    <img src="https://i.loli.net/2021/09/16/NGkQV7TY3oP5qZ9.png" alt="image-20210915100155248.png" style="zoom:67%;" />

73. 【JavaScript】keys 默认按编写顺序遍历对象属性。
...
74. 【Vue】$options：vue 中 export default 的是一个大的选项，通过 $options 可以拿到其中的选项值。
    ...
    80. 【Vue】addRoutes 导致路由重复创建：[vue-router 动态添加路由的方法，addRouter 添加路由，提示：Duplicate named routes definition-Bug 收集 - Bug 搜集 (bugshouji.com)](http://www.bugshouji.com/mybug3/t860)

    81. 【Vue】[解决 NavigationDuplicated: Avoided redundant navigation to current location: “/“\_卡尔特斯-CSDN 博客](https://blog.csdn.net/zz00008888/article/details/119566375)
    ...
    83. 【uniapp】不支持动态组件 [uniapp 暂不支持动态组件，那么有什么替代方案？ - DCloud 问答](https://ask.dcloud.net.cn/question/59079)

    84. 【uniapp】子组件修改父组件传来的 props 数组中的对象中的数组，页面未同步更新。需要 .sync 修饰符

        另外关于一些深度监听的点：[Vue 组件深度(deep)监听 props 对象属性无效的解决办法\_随风丶逆风的博客-CSDN 博客](https://blog.csdn.net/sinat_36521655/article/details/103811630)
    ...
    87. 【Vue】组件无法监听 scroll 事件：[vue 无法监听 scroll 事件 - SegmentFault 思否](https://segmentfault.com/q/1010000009119633)

    88. 【uniapp】无法对 view 和 text 等内置组件使用 ref，自定义组件可以。[uni-app 项目中发现$refs 用不了 - DCloud 问答](https://ask.dcloud.net.cn/question/64975)

    89. 【eslint】eslint wx is not defined：在 eslintrc.js 的 global 变量中添加 wx。 ['wx' is not defined no-undef\_有梦想的程序猿的博客-CSDN 博客](https://blog.csdn.net/weixin_38694789/article/details/104548252)

    90. 【uniapp】swiper 根据内容高度自适应 [uniapp 中如何设置 swiper 的高度自适应的问题解决 - 简书 (jianshu.com)](https://www.jianshu.com/p/6a2fa0f1f86e)

    91. 【JavaScript】获取高度有误：数据未加载完全，可以通过 async await 等调用数据的方法完成后再获取高度。再配合 nextTick()。

    92. 【uniapp】滑动到底部事件 onReachBottom 不生效 [onReachBottom 不起效【已解决】\_小飞飞 ¹²³ 的博客-CSDN 博客](https://blog.csdn.net/qq_44977477/article/details/104924766?utm_medium=distribute.pc_relevant.none-task-blog-2~default~CTRLIST~default-2.no_search_link&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2~default~CTRLIST~default-2.no_search_link)

    93. 【JavaScript】判断字符串是不是纯数字：`isNaN(str)` 当结果是 false 时，表示是 str 是数字，为 true 时，表示 str 不是数字。（注意，当 str 是 null 或""时，值是 false）
    ...
    106.  【Vue】`[Vue warn]: Failed watching path: "columns[6].editSilingHeader" Watcher only accepts simple dot-delimited paths. For full control, use a function instead.`
          ...
          [Vue $watch\_如果有了可惜-CSDN 博客](https://blog.csdn.net/jufjzq/article/details/107786849) watch 不能监听数组子元素的变化
    ...
    116.  【Vue】$attrs与$listeners 的详解：[Vue-- $attrs与$listeners 的详解 - 简书 (jianshu.com)](https://www.jianshu.com/p/a388d38f8c69)
    ...
    120.  【uniapp】onload 和 created 在 h5 和小程序端不同的先后执行顺序https://github.com/dcloudio/uni-app/issues/2572

    121.  【json-server】post 数据只传入 id，content-type: 'application/json;' 不要加分号

    122.  【JavaScript】forEach 与 async/await 坑
    ...
    126.  【Vue】使用 extends 将弹窗封装为全局方法进行调用。[通过 vue.extend 实现消息提示弹框 - 掘金 (juejin.cn)](https://juejin.cn/post/6914239652136714247)

    127.  【Typescript】[Typescript: type‘string’|‘undefined’类型不能赋值给 type‘string’。 - 问答 - 腾讯云开发者社区-腾讯云 (tencent.com)](https://cloud.tencent.com/developer/ask/sof/252348)

    128.  【CSS】[【转】CSS 实现 div 的高度填满剩余空间 - yoyo002 - 博客园 (cnblogs.com)](https://www.cnblogs.com/yoyogis/p/4040513.html)
    ...
    132.  【Html】a 标签嵌套 a 标签：[前端页面 a 标签嵌套 a 标签效果的两种解决方案 - leung_blog - 博客园 (cnblogs.com)](https://www.cnblogs.com/leungUwah/p/6234832.html)
    ...
    143. 【JavaScript】很高级的对象转数组再转对象转数组

    ```js
    const res = {
      Route_num: "16",
      DST_IP0: "192.168.254.12",
      Sub_mask0: "255.255.255.255",
      Gateway0: "192.168.254.254",
      Device0: "nas0_0",
      metric0: "0",
      DST_IP1: "192.168.10.33",
      Sub_mask1: "255.255.255.255",
      Gateway1: "192.168.254.254",
      Device1: "nas0_0",
      metric1: "0"
    };
    const totalKeys = Object.keys(res) || [];
    const standardKeys = ["DST_IP", "Sub_mask", "Gateway", "Device", "metric"];
    const keyMapper = standardKeys.reduce(
      (prev, key) => ({
        ...prev,
        [key]: totalKeys.filter(item => item.includes(key))
      }),
      {}
    );
    state.tableData = Array.from({ length: res.Route_num || 0 }, (_, i) =>
      standardKeys.reduce(
        (obj, key) => ({ ...obj, [key]: res[keyMapper[key][i]] }),
        {}
      )
    );
    ```
    ...
    145. 【CSS】对父元素采用 flex 布局，对子元素配置 `flex: 1`时，默认情况下子元素的 min-width 是 auto，这意味着子元素的内容会强制保持其最小宽度，这可能会导致内容超出父元素的宽度。通过将子元素的 min-width 设置为 0，可以让浏览器缩小子元素的最小宽度，从而允许它收缩到父元素的大小范围内，能够配合 `overflow-x: auto` 实现滚动效果。（方向为 column 时则设置 min-height）

    146. 【CSS】问题：div 里放了一个 img。没有对 div 设置高度，只对图片设置了高度，但是 div 的高度比 img 高。

    原理在于 HTML 中 `img` 元素默认是 `inline` 元素，而 `inline` 元素的布局行为与块级元素不同。
    1) **`inline` 元素的特性**：`inline` 元素不会破坏文档流，也就是说它不会独占一行，而是与其他元素并排显示。而且 `inline` 元素的高度会受到行内基线（baseline）的影响。
    2) **行高和基线对齐**：由于 `img` 是 `inline` 元素，它的默认行为是与文字的基线对齐。这意味着，`img` 元素的下方会有额外的空间，通常是为了容纳字体的下行部分（例如字符的下标或字体的“降部”），即使 `img` 本身没有文字。
    3) **`div` 和 `img` 的关系**：当 `img` 作为 `inline` 元素时，它并没有完全填满父容器 `div` 的高度，而是让父容器的高度变得比图片高一些，通常是因为上述的基线对齐带来的空隙。
    ### 解决方法：
    - **设置 `img` 为 `block`**：将 `img` 的 `display` 属性设置为 `block`，让它变成块级元素，块级元素会自动填满父元素的高度，不再受到行内基线对齐的影响。
    - **设置 `line-height: 0`**：给父容器 `div` 设置 `line-height: 0`，这会移除父容器的额外空隙，因为父容器 `div` 默认也有一些与行内元素相关的行高。
    这两种方式都可以避免 `img` 元素产生多余的空间。
